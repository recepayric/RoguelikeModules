...
namespace MVC.Base.Runtime.Concrete.Injectable.Provider
{
    public class ProcessProvider : IProcessProvider
    {
        ...
        protected Dictionary<Type, HashSet<Abstract.Injectable.Process.Process>> _activeDictionary = new Dictionary<Type, HashSet<Abstract.Injectable.Process.Process>>(); 
        protected Dictionary<Type, List<Abstract.Injectable.Process.Process>> _inactiveDictionary = new Dictionary<Type, List<Abstract.Injectable.Process.Process>>(); 

        //This will create or get an inactive one.
        public T Get<T>() where T : Abstract.Injectable.Process.Process, new()
        {
            T instance;
            Type type = typeof(T);
        ...
        public bool Return<T>(T process) where T: Abstract.Injectable.Process.Process
        {
            Type type = typeof(T);
            ...
            if (_activeDictionary.ContainsKey(type))
            {
                if (_activeDictionary[type].Contains(process))
                {
                    ...
                    if (!_inactiveDictionary.ContainsKey(type))
                    {
                        _inactiveDictionary.Add(type, new List<Abstract.Injectable.Process.Process>());
                    }
        ...
        private void Create<T>() where T : Abstract.Injectable.Process.Process, new()
        {
            T instance = new T();
            Type type = typeof(T);
            ...
            if (!_activeDictionary.ContainsKey(type))
            {
                _activeDictionary.Add(type,new HashSet<Abstract.Injectable.Process.Process>());
            }
