...
namespace strange.framework.impl
{
    public class Binder : IBinder
    {
        ...
        virtual public void Unbind(IBinding bind)
        {
            if (bind == null)
            {
                return;
            ...
            if (bind.key.GetType().IsArray)
            {
                var array = bind.key as object[];
                foreach (var item in array)
                {
                    Unbind(item, bind.name);
                }
            }
            else
            {
                Unbind(bind.key, bind.name);
            }
        }
        
        
        virtual public void UnbindAll()
        {
            ...
            foreach (var pair in bindings)
            {
                ...
                Type mainKeyType = pair.Key as Type;
