...
namespace strange.extensions.command.impl
{
	public class CommandBinder : Binder, ICommandBinder, IPooledCommandBinder, ITriggerable
	{
		...
		public ICommandBinding CreateCustomBinding()
		{
			return new CommandBinding();
		}

		public void DispatchCustomCommand(ICommandBinding commandBinding, params object[] data)
		{
			var dataList = data as object[];

			object[] values = commandBinding.value as object[];
			Type cmd = values [0] as Type;
			ICommand command = invokeCommand (cmd, commandBinding, dataList, 0);
			ReleaseCommand (command);
		}
		
		public void DispatchCustomCommand<TCommand, TSignalType>(params object[] data)
		{
			var dataList = data as object[];
			
			ICommandBinding binding = new CommandBinding();
			binding.To<TCommand>();
			binding.Bind(new Signal<TSignalType>());
			
			object[] values = binding.value as object[];
			Type cmd = values [0] as Type;
			ICommand command = invokeCommand (cmd, binding, dataList, 0);
			ReleaseCommand (command);
		}

		public void DispatchCustomCommand<TCommand, TSignalType1, TSignalType2>(params object[] data)
		{
			var dataList = data as object[];
			
			ICommandBinding binding = new CommandBinding();
			binding.To<TCommand>();
			binding.Bind(new Signal<TSignalType1, TSignalType2>());
		...
		public void DispatchCustomCommand<TCommand>(params object[] data)
		{
			var dataList = data as object[];
			
			ICommandBinding binding = new CommandBinding();
			binding.To<TCommand>();
			binding.Bind(new Signal());
			object[] values = binding.value as object[];
			Type cmd = values [0] as Type;
			ICommand command = invokeCommand (cmd, binding, dataList, 0);
			ReleaseCommand (command);
