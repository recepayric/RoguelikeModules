...
{
    public abstract class MVCView : MonoBehaviour, IView
    {
        [SerializeField]
        protected bool _showDebug;
        
        public virtual bool isInjectable { get; set; }
        ...
        virtual protected void StartContextOperation(MonoBehaviour view, OperationType type, bool finalTry)
        {
            ...
            switch(initType)
            {
                case InititializationType.Bubble : FindContextRecursively(out context);
                    break;
                case InititializationType.Singleton : context = ViewOperationHandler.FirstMVCContext;
                    break;
            }
        ...
        private bool FindContextRecursively(out IContext context)
        {
            ...
            while (trans.parent != null && loopLimiter < LOOP_MAX)
            {
                loopLimiter++;
                trans = trans.parent;
                if (trans.gameObject.GetComponent<IMVCRoot>() != null)
                {
                    IMVCRoot root = trans.gameObject.GetComponent<IMVCRoot>();
                    if(root != null)
                    {
                        if(_showDebug)
        ...
        protected virtual void RemoveRegistration()
        {
            if (registeredWithContext)
            {
                StartContextOperation(this, OperationType.Remove, false);
                registeredWithContext = false;
                var mediator = GetComponent<Mediator>();
                if(mediator != null)
                    Destroy(mediator);
            }
        }
