...
namespace MVC.Base.Editor.Concrete.MVCCodeGeneration.Code
{
    public class CodeUtilities
    {
        ...
        private static string ScriptRoot = "Assets/Scripts/";
        ...
        /// <summary>
        /// This function Will Remove On Next Version
        /// </summary>
        ...
        public static void SaveFileCheckAndReplaceName(string data, string filename)
        {
            if (File.Exists(filename) == false)
            {
                using (StreamWriter outfile = new StreamWriter(
            ...
            else
            {
                ...
                var newName = splites[splites.Length - 1];
                ...
                result += newName + "-Copy-.cs";
                SaveFileCheckAndReplaceName(data, result);
                Debug.LogError(
                    "SAME CLASS EXISTS - System Just Create Class WITH COPY NAME YOU SHOUD FIX IT CLASS NAME IS = " +
                    oldName + ".cs");
            }
        }

        public static bool CheckFileIfExists(string filename)
        {
